"""
Parallel PINN (DDP) for the 1D Buckley-Leverett equation, Adam only.

The physics, the model and the metrics are taken from train_bl.py unchanged.
The single difference is that full-batch Adam is replaced by mini-batch Adam
parallelised with DistributedDataParallel.

Experiment: 3 learning rate scaling laws x 8 mini-batch sizes = 24
configurations. Each one performs a fixed number of optimizer updates (not
epochs), 5000 or 15000 depending on --adam-steps.
...
Usage:
    torchrun --nproc_per_node=2 train_ddp.py --verify   # single configuration
    torchrun --nproc_per_node=2 train_ddp.py            # all 24 configurations
    python train_ddp.py --baseline-reference            # 1 GPU, full batch
"""
...
EXPERIMENTS_DIR = "ddp_adam/15000"
DATA_DIR = "data"
